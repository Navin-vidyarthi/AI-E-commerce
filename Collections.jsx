...
function Collections() {
  ...
  return (
    <div className='w-[100%] min-h-[100%] bg-gradient-to-l from-[#0b1c1d] to-[#144d53]
        ...
        <div className={`border-[2px] border-[#dedcdc] pl-5 py-3 mt-6 rounded-md bg-slate-600 ${showFilter ? '' : 'hidden'} md:block`}>
          ...
          <div className='w-[230px] h-[120px] flex items-start justify-center gap-[10px] flex-col'>
            {['Men', 'Women', 'Kids'].map(category => (
              <label key={category} className='flex items-center gap-[10px] text-[16px] font-light'>
                <input type='checkbox' value={category} className='w-3' onChange={toggleCategory} /> {category}
              </label>
            ))}
          </div>
        </div>
