...
function Nav() {
  ...
  return (
    <div className='w-[100vw] h-[70px] bg-gradient-to-l from-[#2c2f3a] to-[#162e45] z-10 fixed
    top-0 flex items-center justify-between px-[30px] shadow-md  '>
        <div className='w-[20%] lg:w-[30%] flex items-center justify-start gap-[10px]'>
            <img src={logo} alt='' className='w-[30px]'/>
            <h1 className='text-[25px] text-[#fff] font-sans'>AICart</h1>
        ...
        <div className='w-[30%]  flex items-center justify-end gap-[20px]'>
            <IoSearchCircleOutline className='w-[38px] h-[38px] text-[#fff] cursor-pointer' onClick={()=>{setShowSearch(prev=>!prev);navigate("/collection")}}/>
           {!userData && <FaCircleUser className='w-[30px] h-[30px] text-[#fffafa] cursor-pointer' onClick={()=>setShowProfile(prev=>!prev)} />}
           ...
           justify-center cursor-pointer' onClick={()=>setShowProfile(prev=>!prev)}>{userData?.name.slice(0,1)}</div>}
            <TiShoppingCart className='w-[38px] h-[38px] text-[#ebf1f2] cursor-pointer hidden md:block' onClick={()=>navigate("/cart")}/>
        ...
        absolute top-[100%] left-0 right-0 flex items-center justify-center'>
            ...
            placeholder:text-white text-18px' placeholder='Search Here' onChange={(e)=>{setSearch(e.target.value)}} value={search} />
        ...
        {showProfile && <div className='absolute w-[220px] h-[150px] bg-[#000000d7] top-[110%] right-[4%] border-[1px] border-[#aaa9a9] rounded-[10px] z-10'>
            <ul className='w-[100%] h-[100%] flex items-start justify-around flex-col text-[17px] py-[10px] text-[white]'>
                ...
                onClick={()=>{handaleLogout();navigate("/");setShowProfile(false)}} >LogOut</li>}
                <li className='w-[100%] hover:bg-[#2f2f2f] px-[15px] py-[10px] cursor-pointer' 
                onClick={()=>navigate('/order')}>Order</li>
                <li className='w-[100%] hover:bg-[#2f2f2f] px-[15px] py-[10px] cursor-pointer'
        ...
        <div className='w-[100vw] h-[70px] flex items-center justify-between
        px-[20px] text-[12px] fixed bottom-0 left-0 bg-[#191818] md:hidden'>
            <button className='text-[white] flex items-center
            justify-center flex-col gap-[10px]' onClick={()=>navigate("/")}><IoMdHome className='w-[20px] h-[20px] text-[white] md:hidden' />HOME</button>
            <button className='text-[white] flex items-center
            justify-center flex-col gap-[10px]' onClick={()=>navigate("/collection")}><BsFillCollectionFill  className='w-[20px] h-[20px] text-[white] md:hidden' />Collections</button>
            <button className='text-[white] flex items-center
            justify-center flex-col gap-[10px]' onClick={()=>navigate("/contact")}><MdOutlineContacts className='w-[20px] h-[20px] text-[white] md:hidden' />Contact</button>
            <button className='text-[white] flex items-center
            justify-center flex-col gap-[10px]' onClick={()=>navigate("/cart")}><TiShoppingCart className='w-[20px] h-[20px] text-[white] md:hidden' />Cart</button>
            <p className='absolute w-[18px] h-[18px] flex items-center justify-center
            bg-white px-[5px] py-[2px] text-black font-semibold rounded-full text-[9px] top-[8px] right-[18px]'>{getCartCount()}</p>
