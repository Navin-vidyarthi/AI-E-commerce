...
function Lists() {
  ...
  const fetchList = async () => {
    try {
      ...
      toast.success('Listed Product Successfully')
    } catch (error) {
      console.log(error);
      toast.error('Listed Product Failed')
    }
  };
  ...
  return (
    <div className='w-[100vw] h-[100vw] bg-gradient-to-l from-[#141414] to-[#0c2025] text-white m-l-5'>
      <Nav />
      <div className='w-full h-full flex items-center justify-start'>
        ...
        <div className='w-[82%] h-full lg:ml-[320px] md:ml-[230px] mt-[70px] flex flex-col gap-[30px] overflow-x-hidden py-[50px] ml-[100px]'>
          <div className='w-[400px] h-[50px] text-[25px] md:text-[40px] py-[40px] ml-[40px]'>All Listed Product</div>

          {list?.length > 0 ? (
            list.map((item, index) => (
              <div
                key={index}
                className='w-[90%] md:h-[120px] h-[90px] bg-slate-600 rounded-xl flex items-center justify-start gap-[5px] md:gap-[30px] m-[10px] p-[10px] md:px-[30px]  ml-[40px]'>
                <img
                  src={item.image1}
                  alt={item.name}
                  className='w-[30%] md:w-[120px] h-[90%] rounded-lg'
                />
                <div className='w-[90%] h-[80%] flex mb-[50px] items-end justify-center gap-[2px] relative'>
                  <div className='w-full md:text-[20px] text-[15px] text-[#bef0f3]'>{item.name}</div>
                  <div className='md:text-[17px] text-[15px] text-[#bef3da] items-center mr-[50%]'>{item.category}</div>
