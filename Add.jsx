...
function Add() {
    ...
    const [subCategort, setSubCategory] = useState("TopWear")
    ...
    try {
        ...
        formData.append("subCategory", subCategort); // fixed
  ...
  return (
    <div className='w-[100vw] h-[100vw] bg-gradient-to-l from-[#1a1a2e] to-[#16213e] text-[white] relative'>
        <Nav/>
        <Sidebar/>
      <div className='w-[82%] h-[100%] flex items-center justify-start overflow-x-hidden absolute right-0'>
        ...
        py-[60px] px-[30px] md:px-[60px]'>
            <div className='w-[400px] h-[50px] text-[25px] md:tetx-[40px]
            ...
            <div className='w-[80%] h-[100px] flex items-start justify-center flex-col gap-[10px]'>
                ...
                <input type='text' placeholder='Type here' className='w-[600px] max-w-[98%] h-[48px] rounded-lg hover:border-[#46d1f7] border-[2px] cursor-pointer bg-slate-600 px-[20px] text-[18px] placeholder:text-[#ffffffc2]'onChange={(e)=>setName(e.target.value)} value={name} required/>
            ...
            <div className='w-[80%] h-[100px] flex items-start justify-center flex-col gap-[10px]'>
                ...
                <input type='number' placeholder= '₹ 2000' className='w-[600px] max-w-[98%] h-[48px] rounded-lg hover:border-[#46d1f7] border-[2px] cursor-pointer bg-slate-600 px-[20px] text-[18px] placeholder:text-[#ffffffc2]'
                ...
                <div className={`px-[20px] py-[7px] rounded-lg bg-slate-600 text-[18px] hover:border-[#46f7f1] border[2px] cursor-pointer ${sizes.includes("S") ? "bg-green-500 text-black border-[#46d1f7]" : ""}`} onClick={()=>setSizes(prev => prev.includes("S") ? prev.filter(item =>item !== "S") : [...prev ,"S"])}>S</div>

                <div className={`px-[20px] py-[7px] rounded-lg bg-slate-600 text-[18px] hover:border-[#46f7f1] border[2px] cursor-pointer ${sizes.includes("M") ? "bg-green-500 text-black border-[#46d1f7]": ""}`} onClick={()=>setSizes(prev => prev.includes("M") ? prev.filter(item =>item !== "M") : [...prev ,"M"])}>M</div>


                <div className={`px-[20px] py-[7px] rounded-lg bg-slate-600 text-[18px] hover:border-[#46f7f1] border[2px] cursor-pointer ${sizes.includes("L") ? "bg-green-500 text-black border-[#46d1f7]": ""}`} onClick={()=>setSizes(prev => prev.includes("L") ? prev.filter(item =>item !== "L") : [...prev ,"L"])}>L</div>

                <div className={`px-[20px] py-[7px] rounded-lg bg-slate-600 text-[18px] hover:border-[#46f7f1] border[2px] cursor-pointer ${sizes.includes("XL") ? "bg-green-500 text-black border-[#46d1f7]": ""}`} onClick={()=>setSizes(prev => prev.includes("XL") ? prev.filter(item =>item !== "XL") : [...prev ,"XL"])}>XL</div>

                <div className={`px-[20px] py-[7px] rounded-lg bg-slate-600 text-[18px] hover:border-[#46f7f1] border[2px] cursor-pointer ${sizes.includes("XXL") ? "bg-green-400 text-black border-[#46d1f7]": ""}`} onClick={()=>setSizes(prev => prev.includes("XXL") ? prev.filter(item =>item !== "XXL") : [...prev ,"XXL"])}>XXL</div>
                </div>
              ...
              <div className='w-[80%] flex items-center justify-start gap-[10px] mt-[20px]'>
                ...
                <button className='w-[140px] px-[20px] py-[20px] rounded-2xl bg-[#65d8f7] flex items-center justify-center gap-[10px]text-black active:bg-slate-700 active:text-white active:border-[2px] border-white border-r-2 cursor-pointer'>{loading ? <Loading/> : "Add Product"}</button>
            </form>
